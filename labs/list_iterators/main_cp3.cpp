...
// Using only front(), pop_front(), and insert().
void reverseFrames(std::list<GifFrame>& frames) {
	typename std::list<GifFrame>::iterator itr = frames.end();
	for (unsigned int i = 0; i < frames.size(); i++) {
        GifFrame temp = frames.front();
        frames.pop_front();
        itr = frames.insert(itr, temp);
    }
}
// Using only back(), pop_back(), insert(), and iterator increment. ./
// void reverseFrames(std::list<GifFrame>& frames) {
// 	typename std::list<GifFrame>::iterator itr;
// 	for (itr = frames.begin(); itr != --frames.end(); ++itr) {
// 		GifFrame temp = frames.back();
// 		frames.pop_back();
// 		frames.insert(itr, temp);
//         itr--;
//     }
// }
// Using only erase(), push_front(), and iterator dereference. ./
// void reverseFrames(std::list<GifFrame>& frames) {
// 	typename std::list<GifFrame>::iterator itr = frames.begin();
...
// 	}
// }
// Using only erase(), push_back(), iterator dereference, and iterator decrement. ./
// void reverseFrames(std::list<GifFrame>& frames) {
// 	typename std::list<GifFrame>::iterator itr = --frames.end();
// 	while (itr != frames.begin()){
// 		itr--;
// 		GifFrame temp = *itr;
// 		itr = frames.erase(itr);
